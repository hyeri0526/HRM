...
namespace LhrMain._02인사기록관리
{
    ...
    /// 1. Project        : 교육용 시스템
    /// 2. 업무분류       : 업무1
    /// 3. 업무명칭       : 자격면허 (1:N CRUD)
    /// 4. 최초작성 정보  : 2023.04.25 / 이혜리
    /// 5. 최근수정 정보  : 2023.04.25 / 이혜리
    ///**************************************************************************
    /// </summary>
    /// 
    public partial class 자격면허 : Form
    {
        #region 초기 설정
        ...
        private void 자격면허_Load(object sender, EventArgs e)
        {
        ...
        public void BtnSearch_Click()
        {
            ...
            try
            {
                ...
                cmd.CommandText = SQL.SQLStatement7.SelectSQL0;
                cmd.BindByName = true;
        ...
        public void BtnSearch2_Click()
        {
            ...
            try
            {
                con = Utility.SetOracleConnection();
                OracleCommand cmd = con.CreateCommand();
                cmd.CommandText = SQL.SQLStatement7.SelectSQL;
                cmd.BindByName = true;
                cmd.Parameters.Add("lic_empno", OracleDbType.Varchar2).Value = ct_bas_empno.Text;
                cmd.Parameters.Add("lic_name", OracleDbType.Varchar2).Value = ct_lic_name.Text;
        ...
        public void BtnInsert_Click()
        {
            ...
            ct_lic_name.Focus();                                    //* 첫번째 입력 콘트롤에 Focusing    
        ...
        public void BtnDelete_Click()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = con.CreateCommand();
                cmd.BindByName = true;
                cmd.CommandText = SQL.SQLStatement7.DeleteSQL;
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    {
                        cmd.CommandText = SQL.SQLStatement7.InsertSQL;
                        cmd.Parameters.Add("lic_empno", OracleDbType.Varchar2).Value = row.Cells["lic_empno"].Value;
                    }
                    ...
                    cmd.Parameters.Add("lic_acqdate", OracleDbType.Varchar2).Value = Utility.FormatDateR((string)row.Cells["lic_acqdate"].Value);
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            if (ct_lic_name.Text == "")
            {
                SetError(ct_lic_name, "급수를 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_lic_name, errorProvider1);
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (Utility.Check_Date_IsEmpty(ct_lic_acqdate.Text))
            {
                SetError(ct_lic_acqdate, "취득일을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else if (Utility.Check_Date_YYYYMMDD(ct_lic_acqdate.Text) == false)
            {
                SetError(ct_lic_acqdate, "취득일 입력 형태가 올바르지 않습니다.", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
