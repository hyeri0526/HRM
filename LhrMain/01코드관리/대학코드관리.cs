...
namespace LhrMain._01코드관리
{
    ...
    ///**************************************************************************
    /// </summary>
    public partial class 대학코드관리 : Form
    {
        ...
        public 대학코드관리()
        {
            ...
            ct_univ_code.TextChanged += InputData_TextChanged;
            ct_univ_name.TextChanged += InputData_TextChanged;            
            ct_univ_seq.TextChanged += InputData_TextChanged;
            ct_univ_use.CheckedChanged += InputData_TextChanged;

            //*---Validated Event Handler(Start) 유효성 검사 ------------
            ct_univ_code.Validated += Input_Validation_Check;
            ct_univ_name.Validated += Input_Validation_Check;
            ct_univ_seq.Validated += Input_Validation_Check;
        }

        private void 대학코드관리_Load(object sender, EventArgs e)
        {
        ...
        public void BtnInsert_Click()
        {
            ...
            ct_univ_code.Focus();                                   //최초 입력 컨트롤(사원번호)로 이동
        ...
        public void BtnSearch_Click()
        {
            ...
            try
            {
                ...
                cmd.CommandText = SQL.SQLStatement.SelectUnivCode;
                cmd.BindByName = true;                                        //바인드 변수를 사용할 때 이름으로 바인딩하도록 설정
                cmd.Parameters.Add("univ_name", OracleDbType.Varchar2).Value = txt_Search.Text + "%";
                ...
                while (dr.Read())
                {
                    ...
                    row.Cells["univ_code"].Value = dr["univ_code"].ToString();
                    row.Cells["univ_name"].Value = dr["univ_name"].ToString();
                    row.Cells["univ_seq"].Value = dr["univ_seq"].ToString(); ;
                    row.Cells["univ_use"].Value = Utility.ChangeBoolType(dr["univ_use"].ToString());
        ...
        public void BtnDelete_Click()
        {
            ...
            DialogResult result = MessageBox.Show(row.Cells["univ_code"].Value +
            ...
            try
            {
                ...
                OracleCommand cmd = con.CreateCommand();
                cmd.BindByName = true;
                cmd.CommandText = SQL.SQLStatement.DeleteUnivCode;
                cmd.Parameters.Add("univ_code", OracleDbType.Varchar2).Value = row.Cells["univ_code"].Value;
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    {
                        cmd.CommandText = SQL.SQLStatement.InsertUnivCode;
                    }
                    if (row.Cells["status"].Value.Equals("U"))                                                  //* if (row.Cells["status"].Value.Equals("U"))
                    {
                        cmd.CommandText = SQL.SQLStatement.UpdateUnivCode;
                    }

                    //                   
                    cmd.Parameters.Add("univ_code", OracleDbType.Varchar2).Value = row.Cells["univ_code"].Value;
                    cmd.Parameters.Add("univ_name", OracleDbType.Varchar2).Value = row.Cells["univ_name"].Value;
                    cmd.Parameters.Add("univ_seq", OracleDbType.Varchar2).Value = row.Cells["univ_seq"].Value;
                    cmd.Parameters.Add("univ_use", OracleDbType.Varchar2).Value = Utility.ChangeBoolType(row.Cells["univ_use"].Value, "YN");
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (ct_univ_code.Text == "")
            {
                SetError(ct_univ_code, "대학코드를 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_univ_code, errorProvider1);
            }

            //*---------------------------------------------------------------------------------------------------------
            if (ct_univ_name.Text == "")
            {
                SetError(ct_univ_name, "대학명을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_univ_name, errorProvider1);
            }
                        
            //*---------------------------------------------------------------------------------------------------------
            if (ct_univ_seq.Text == "")
            {
                SetError(ct_univ_seq, "코드SEQ를 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_univ_seq, errorProvider1);
            }
        }
