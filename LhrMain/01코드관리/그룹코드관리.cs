...
{    
    ...
    /// 2. 업무분류       : 시스템
    /// 3. 업무명칭       : 그룹코드관리
    /// 4. 최초작성 정보  : 2023.04.04 / 이혜리
    /// 5. 최근수정 정보  : 2023.04.04 / 이혜리
    ///******************************************************************
    /// </summary>
    /// 
    public partial class 그룹코드관리 : Form
    {
        #region 초기 설정
        ...
        private void 그룹코드관리_Load(object sender, EventArgs e)
        {
        ...
        public void BtnSearch_Click()
        {
            ...
            try
            {
                ...
                cmd.CommandText = SQL.SQLStatement.SelectGcode;
                cmd.BindByName = true;                                        //바인드 변수를 사용할 때 이름으로 바인딩하도록 설정
                cmd.Parameters.Add("cdg_grpnm", OracleDbType.Varchar2).Value = txt_Search.Text + "%";
                ...
                while (dr.Read())
                {
                    ...
                    row.Cells["cdg_digit"].Value = dr["cdg_digit"];
                    row.Cells["cdg_length"].Value = dr["cdg_length"];
        ...
        public void BtnInsert_Click()
        {
            ...
            ct_cdg_grpcd.Focus();                                   //최초 입력 컨트롤(사원번호)로 이동
        ...
        public void BtnDelete_Click()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = con.CreateCommand();
                cmd.BindByName = true;
                cmd.CommandText = SQL.SQLStatement.DeleteGcode;
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    {
                        cmd.CommandText = SQL.SQLStatement.InsertGcode;
                    }
                    if (row.Cells["status"].Value.Equals("U"))                                                  //* if (row.Cells["status"].Value.Equals("U"))
                    {
                        cmd.CommandText = SQL.SQLStatement.UpdateGcode;
                    }
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            {
                SetError(ct_cdg_digit, "단위코드 길이를 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
            ...
            {
                SetError(ct_cdg_length, "단위코드명 길이를 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
