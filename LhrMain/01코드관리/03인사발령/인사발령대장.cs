...
namespace LhrMain._03인사발령
{
    ...
    /// 2. 업무분류       : 시스템
    /// 3. 업무명칭       : 인사발령대장
    /// 4. 최초작성 정보  : 2023.04.20 / 이혜리
    /// 5. 최근수정 정보  : 2023.04.20 / 이혜리
    ///******************************************************************
    /// </summary>
    /// 
    public partial class 인사발령대장 : Form
    {
        #region 초기 설정
        ...
        private void 인사발령대장_Load(object sender, EventArgs e)
        {
        ...
        public void BtnSearch_Click()
        {
            ...
            try
            {
                ...
                cmd.CommandText = SQL.SQLStatement4.SelectPAPR;
        ...
        public void BtnInsert_Click()
        {
            ...
            ct_papr_appno.Focus();                                   //최초 입력 컨트롤(사원번호)로 이동
        ...
        public void BtnDelete_Click()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = con.CreateCommand();
                cmd.BindByName = true;
                cmd.CommandText = SQL.SQLStatement4.DeletePAPR;
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    {
                        cmd.CommandText = SQL.SQLStatement4.InsertPAPR;
                    }
                    if (row.Cells["status"].Value.Equals("U"))                                                  //* if (row.Cells["status"].Value.Equals("U"))
                    {
                        cmd.CommandText = SQL.SQLStatement4.UpdatePAPR;
                    }
                    
                    cmd.Parameters.Add("papr_appno", OracleDbType.Varchar2).Value = row.Cells["papr_appno"].Value;
                    cmd.Parameters.Add("papr_date", OracleDbType.Varchar2).Value = Utility.FormatDateR((string)row.Cells["papr_date"].Value);
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            {
                SetError(ct_papr_appno, "인사발령번호를 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
            ...
            //*---------------------------------------------------------------------------------------------------------           
            if (Utility.Check_Date_IsEmpty(ct_papr_date.Text))
            {
                SetError(ct_papr_date, "시행일자를 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else if (Utility.Check_Date_YYYYMMDD(ct_papr_date.Text) == false)
            {
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (ct_papr_content.Text == "")
            {
                SetError(ct_papr_content, "발령내용을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_papr_content, errorProvider1);
            }
