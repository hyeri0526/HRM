...
namespace LhrMain._02인사기록관리
{
    ...
    /// 1. Project        : 교육용 시스템
    /// 2. 업무분류       : 업무1
    /// 3. 업무명칭       : 학력사항 (1:N CRUD)
    /// 4. 최초작성 정보  : 2023.04.25 / 이혜리
    /// 5. 최근수정 정보  : 2023.04.25 / 이혜리
    ///**************************************************************************
    /// </summary>
    /// 
    public partial class 학력사항 : Form
    {
        #region 초기 설정
        ...
        public 학력사항()
        {
            ...
            ct_edu_schnm.Validated += Input_Validation_Check;
            ct_edu_entdate.Validated += Input_Validation_Check;
            ct_edu_gradate.Validated += Input_Validation_Check;
            ct_edu_gra.Validated += Input_Validation_Check;
            //*---Validated Event Handler(End)------------------------------
        }

        private void 학력사항_Load(object sender, EventArgs e)
        {
        ...
        public void BtnSearch_Click()
        {
            ...
            try
            {
                ...
                cmd.CommandText = SQL.SQLStatement6.SelectSQL0;
                cmd.BindByName = true;
        ...
        public void BtnSearch2_Click()
        {
            ...
            try
            {
                con = Utility.SetOracleConnection();
                OracleCommand cmd = con.CreateCommand();
                cmd.CommandText = SQL.SQLStatement6.SelectSQL;
                cmd.BindByName = true;
                cmd.Parameters.Add("edu_empno", OracleDbType.Varchar2).Value = ct_bas_empno.Text;
                cmd.Parameters.Add("edu_loe", OracleDbType.Varchar2).Value = ct_edu_loe.Text;
                ...
                while (dr.Read())
                {
                    ...
                    row.Cells["edu_grade"].Value = String.Format("{0:0.00}", dr["edu_grade"]);
        ...
        public void BtnInsert_Click()
        {
            ...
            ct_edu_loe.Focus();                                    //* 첫번째 입력 콘트롤에 Focusing    
        ...
        public void BtnDelete_Click()
        {
            ...
            DialogResult result = MessageBox.Show(row.Cells["emp_loe"].Value +
            ...
            try
            {
                ...
                OracleCommand cmd = con.CreateCommand();
                cmd.BindByName = true;
                cmd.CommandText = SQL.SQLStatement6.DeleteSQL;
        ...
        public void BtnSave_Click()
        {
            ...
            try
            {
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    {
                        cmd.CommandText = SQL.SQLStatement6.InsertSQL;
                        cmd.Parameters.Add("edu_empno", OracleDbType.Varchar2).Value = row.Cells["edu_empno"].Value;
                    }
                    else   //*  if (row.Cells["status"].Value.Equals("U"))
                    {
                        cmd.CommandText = SQL.SQLStatement6.UpdateSQL;
                        cmd.Parameters.Add("edu_empno", OracleDbType.Varchar2).Value = row.Cells["edu_empno"].Value;
                        cmd.Parameters.Add("edu_loe", OracleDbType.Varchar2).Value = row.Cells["edu_loe"].Value;
                    }
                    ...
                    cmd.Parameters.Add("edu_entdate", OracleDbType.Varchar2).Value = Utility.FormatDateR((string)row.Cells["edu_entdate"].Value);
                    cmd.Parameters.Add("edu_gradate", OracleDbType.Varchar2).Value = Utility.FormatDateR((string)row.Cells["edu_gradate"].Value);
        ...
        private void Input_Validation_Check(object sender, EventArgs e)
        {
            ...
            if (ct_edu_loe.Text == "")
            {
                SetError(ct_edu_loe, "학력구분을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (ct_edu_schnm.Text == "")
            {
                SetError(ct_edu_schnm, "학교명을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_edu_schnm, errorProvider1);
            }
            ...
            if (ct_edu_gra.Text == "")
            {
                SetError(ct_edu_gra, "졸업구분을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
                ResetError(ct_edu_gra, errorProvider1);
            }

            //*---------------------------------------------------------------------------------------------------------
            if (Utility.Check_Date_IsEmpty(ct_edu_entdate.Text))
            {
                SetError(ct_edu_entdate, "근무종료월을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else if(Utility.Check_Date_YYYYMMDD(ct_edu_entdate.Text) == false)
            {
                SetError(ct_edu_entdate, "입학일자 입력 형태가 올바르지 않습니다.", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
            ...
            //*---------------------------------------------------------------------------------------------------------
            if (Utility.Check_Date_IsEmpty(ct_edu_gradate.Text))
            {
                SetError(ct_edu_gradate, "근무종료월을 반드시 입력하세요", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else if(Utility.Check_Date_YYYYMMDD(ct_edu_gradate.Text) == false)
            {
                SetError(ct_edu_gradate, "졸업일자 입력 형태가 올바르지 않습니다.", dataGridView1.SelectedRows[0], errorProvider1);
            }
            else
            {
